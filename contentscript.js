      ...
      if ($('#finals_toggle_button').is(':checked')==true){
        ...
        $('#finals_assignment').load(FINALS_HTML, function(){
          if (finals_category !== null){
            ...
            $(this).find('td:nth-child(2)').html('Weight:\n ' + Math.round(finals_category['weight']*100)+'%');
            ...
            $(this).find('#percent_category_final').change(function(){
              finals_category_weight = categories[getAssignmentInfo($('#finals_assignment'))['category']]['weight'];
              if ($(this).val() >= finals_category_weight*100 && is_weighted){
                $(this).val(finals_category_weight*100 - 1);
              } else if($(this).val() <= 0 ){
                $(this).val(1);
              } else if($(this).val() >= 100 && !is_weighted){
                $(this).val(99)
              }
              ...
              $(ASSIGNMENT_TABLE_CSS_PATH +' > tr').not('#menu_assignment, #finals_assignment').each(function(){
                ...
                user_points_unweighted += assignment['user_score'];
                max_points_unweighted += assignment['max_score'];
              });
              if (is_weighted){
                finals_category_weight = categories[getAssignmentInfo($('#finals_assignment'))['category']]['weight'];
                finals_category = getAssignmentInfo($('#finals_assignment'))['category']

                percent_of_category_final = (1/finals_category_weight)*($(this).val()/100);
                console.log(percent_of_category_final);
                $('#finals_max_score').val(Math.round(max_points[finals_category]*(percent_of_category_final)/(1-percent_of_category_final ))) ;
              } else{
                $('#finals_max_score').val(Math.round(max_points_unweighted * ($(this).val()/100)/(1-$(this).val()/100)));
              }
            });
            $(this).find('#finals_max_score').change(function(){
              ...
              finals_category_weight = categories[getAssignmentInfo($('#finals_assignment'))['category']]['weight'];
              if (is_weighted){
                $('#percent_category_final').val(Math.round((finals_category_weight * $(this).val() /max_points[finals_category])* 100))
              } else {
                $('#percent_category_final').val(Math.round(($(this).val()/max_points_unweighted)*100));
              }
            })
          }
        });
